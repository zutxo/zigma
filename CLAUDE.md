...
- **ALWAYS** read `ZIGMA_STYLE.md` before writing code
- **ALWAYS** use `bd` (beads) for task tracking, **NEVER** markdown TODOs
- **ALWAYS** use `./zig/zig` instead of system `zig` (project-managed version, TigerBeetle-style)
- **ALWAYS** run `./zig/zig fmt src/` before committing
- **ALWAYS** run `./zig/zig build test` before committing (all tests must pass)
...
./zig/zig build -Doptimize=ReleaseFast     # Release build
./zig/zig build test                       # Run all tests
./zig/zig build test -- --test-filter "vlq" # Run specific tests

# Development
./zig/zig fmt src/                         # Format code (ALWAYS before commit)
./zig/zig build docs                       # Generate documentation

# Benchmarks
./zig/zig build bench                      # Run benchmarks
./zig/zig build bench -Doptimize=ReleaseFast  # Release benchmarks

# Cross-compilation
./zig/zig build -Dtarget=x86_64-linux-gnu
./zig/zig build -Dtarget=aarch64-linux-gnu
./zig/zig build -Dtarget=wasm32-freestanding
...
zigma/
├── zig/
│   ├── download.sh           # Downloads project-managed Zig
│   └── zig                   # Zig binary (gitignored, run download.sh)
├── src/
...
├── docs/
│   ├── ZIGMA_STYLE.md        # Coding style guide
...
4. **Run full test suite**: `./zig/zig build test`
...
./zig/zig build test -- --test-filter "type_serializer"
...
- [ ] `./zig/zig fmt src/` run
- [ ] `./zig/zig build test` passes (all tests)
